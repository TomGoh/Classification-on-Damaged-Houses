...
if __name__ == '__main__':
    ...
    resnet_history = resnet.fit(train_images, train_labels, batch_size=32, validation_data=(vali_images, vali_labels),
                                epochs=50,
    ...
    np.savetxt('history/ValidationLoss_Cutmix.txt', resnet_history.history['val_loss'])
    np.savetxt('history/ValidationAccuracy_Cutmix.txt', resnet_history.history['val_accuracy'])
    np.savetxt('history/TrainingLoss_Cutmix.txt', resnet_history.history['loss'])
    np.savetxt('history/TrainingAccuracy_Cutmix.txt', resnet_history.history['accuracy'])
    np.savetxt('history/TestResult_Cutmix.txt', np.array([test_loss, test_accuracy, another_test_loss, another_test_accuracy]))
